...
function dbxManager(sid) {
	...
	this.etype = typeof document.addEventListener != 'undefined' ? 'addEventListener'
			: typeof document.attachEvent != 'undefined' ? 'attachEvent'
					: 'none';
	this.eprefix = (this.etype == 'attachEvent' ? 'on' : '');
	if (typeof window.opera != 'undefined'
			&& parseFloat(
					navigator.userAgent.toLowerCase().split(/opera[\/ ]/)[1]
							.split(' ')[0], 10) < 7.5) {
		this.etype = 'none';
	}
	if (this.etype == 'none') {
...
dbxManager.prototype.setCookieState = function() {
	...
	if (typeof this.onstatechange == 'undefined' || this.onstatechange()) {
		document.cookie = 'dbx-' + this.sid + '=' + this.cookiestring
...
dbxManager.prototype.getCookieState = function() {
	...
	if (document.cookie) {
		if (document.cookie.indexOf('dbx-' + this.sid) != -1) {
			this.cookie = document.cookie.split('dbx-' + this.sid + '=')[1]
					.split(';')[0].split('&');
...
dbxManager.prototype.createElement = function(tag) {
	return typeof document.createElementNS != 'undefined' ? document
			.createElementNS('http://www.w3.org/1999/xhtml', tag) : document
			.createElement(tag);
};
...
function dbxGroup(gid, dir, thresh, fix, ani, togs, def, open, close, move,
		toggle, kmove, ktoggle, syntax) {
	if (!/^[-_a-z0-9]+$/i.test(gid)) {
		alert('Error from dbxGroup:\n"' + gid + '" is an invalid container ID');
		return;
	}
	...
	for ( var i = 0; i < this.eles.length; i++) {
		if (/dbx\-box/i.test(this.eles[i].className)
				&& !/dbx\-dummy/i.test(this.eles[i].className)) {
		...
		if (/dbx\-handle/i.test(this.eles[i].className)) {
			...
			this.eles[i].setAttribute('title', this.eles[i]
					.getAttribute('title') == null
					|| this.eles[i].title == '' ? this.vocab.move
					: this.vocab.syntax
							.replace('%mytitle%', this.eles[i].title).replace(
									'%dbxtitle%', this.vocab.move));
			if (this.toggles) {
				this.buttons.push(this.addToggleBehavior(this.eles[i]));
			} else {
				this.eles[i][dbx.etype]
						(
								dbx.eprefix
										+ 'key'
										+ (typeof document.uniqueID != 'undefined'
												|| navigator.vendor == 'Apple Computer, Inc.' ? 'down'
												: 'press'), function(e) {
									if (!e) {
										e = window.event;
									}
									return self.keypress(e, dbx.getTarget(e,
											'dbx\-handle'));
								}, false);
	...
	if (dbx.cookiestate != null
			&& typeof dbx.cookiestate[this.gid] != 'undefined') {
...
dbxGroup.prototype.addToggleBehavior = function() {
	...
	button.setAttribute('title', this.vocab.toggle.replace('%toggle%',
			this.vocab.close));
	button.hasfocus = typeof window.opera != 'undefined'
			|| navigator.vendor == 'Apple Computer, Inc.' ? null : false;
	this.tooltip = null;
	...
	button['onkey' + (typeof document.uniqueID != 'undefined'
			|| navigator.vendor == 'Apple Computer, Inc.' ? 'down' : 'press')] = function(
			e) {
		if (!e) {
			e = window.event;
		}
	...
	button.onfocus = function() {
		...
		for ( var i = 0; i < len; i++) {
			self.buttons[i].className = self.buttons[i].className.replace(
					/[ ](dbx\-toggle\-hilite\-)(open|closed)/, '');
		}
...
dbxGroup.prototype.toggleBoxState = function(button, regen) {
	...
	if ((!isopen && (typeof dbx.onboxopen == 'undefined' || dbx.onboxopen()))
			|| (isopen && (typeof dbx.onboxclose == 'undefined' || dbx
					.onboxclose()))) {
		button.className = 'dbx-toggle dbx-toggle-' + (isopen ? 'closed'
				: 'open');
		button.title = this.vocab.toggle.replace('%toggle%',
				isopen ? this.vocab.open : this.vocab.close);
		if (typeof button.isactive != 'undefined') {
			button.className += ' dbx-toggle-hilite-' + (isopen ? 'closed'
					: 'open')
		}
		parent.className = parent.className.replace(
				/[ ](dbx-box-)(open|closed)/, ' $1' + (isopen ? 'closed'
						: 'open'));
		if (regen) {
			this.getBoxOrder();
		}
...
dbxGroup.prototype.shiftBoxPosition = function(e, anchor, positive) {
	...
	if (typeof dbx.onboxdrag == 'undefined' || dbx.onboxdrag()) {
		...
		for ( var i = 0; i < len; i++) {
			positions[i] = [ i,
					this.boxes[i][this.vertical ? 'offsetTop' : 'offsetLeft'] ];
			...
			if (positions[i][0] == this.idref) {
				if ((positive && i < len - 2) || (!positive && i > 0)) {
					...
					if (this.resolution > 0) {
						...
						var siblingpos = {
							'x' : sibling.offsetLeft,
							'y' : sibling.offsetTop
						};
					}
					var obj = {
						'insert' : (positive ? sibling : parent),
						'before' : (positive ? parent : sibling)
					};
					this.container.insertBefore(obj.insert, obj.before);
					if (this.resolution > 0) {
						var animators = {
							'sibling' : new dbxAnimator(this, sibling,
									siblingpos, this.resolution, true, anchor),
							'parent' : new dbxAnimator(this, parent, visipos,
									this.resolution, true, anchor)
						};
					} else {
						anchor.focus();
					}
	...
	if (this.keydown) {
		...
		if (isopen != null) {
			this.tooltip.appendChild(document.createTextNode(this.vocab.kmove
					+ this.vocab.ktoggle.replace('%toggle%',
							isopen ? this.vocab.close : this.vocab.open)));
		} else {
			this.tooltip.appendChild(document.createTextNode(this.vocab.kmove));
		}
		var parent = dbx.getTarget(null, 'dbx\-box', anchor);
...
dbxGroup.prototype.mousemove = function(e) {
	if (this.dragok && this.box != null) {
		this.positive = this.vertical ? (e.clientY > this.current.y ? true
				: false) : (e.clientX > this.current.x ? true : false);
		this.current = {
			'x' : e.clientX,
			'y' : e.clientY
		};
		...
		if (((overall.x >= 0 && overall.x <= this.threshold) || (overall.x <= 0 && overall.x >= 0 - this.threshold))
		...
		if (this.released || overall.x > this.threshold
				|| overall.x < (0 - this.threshold)
				|| overall.y > this.threshold
				|| overall.y < (0 - this.threshold)) {
			dbx.group = this.container;
			dbx.box = this.box;
			dbx.event = e;
			if (typeof dbx.onboxdrag == 'undefined' || dbx.onboxdrag()) {
...
dbxGroup.prototype.keypress = function(e, anchor) {
	if (/^(3[7-9])|(40)$/.test(e.keyCode)) {
		this.removeTooltip();
		if ((this.vertical && /^(38|40)$/.test(e.keyCode))
				|| (!this.vertical && /^(37|39)$/.test(e.keyCode))) {
			this.shiftBoxPosition(e, anchor,
					/^[3][78]$/.test(e.keyCode) ? false : true);
			if (typeof e.preventDefault != 'undefined') {
				e.preventDefault();
			} else {
				return false;
...
dbxGroup.prototype.getBoxOrder = function() {
	...
	for ( var j = 0; j < len; j++) {
		if (/dbx\-box/i.test(this.eles[j].className)
				&& !/dbx\-(clone|dummy)/i.test(this.eles[j].className)) {
			this.order.push(this.eles[j].className.split('dbxid')[1]
					+ (/dbx\-box\-open/i.test(this.eles[j].className) ? '+'
							: '-'));
		}
	}
...
dbxGroup.prototype.moveOriginalToPosition = function(clientX, clientY) {
	var cloneprops = {
		'xy' : this.vertical ? clientY - this.difference.y : clientX
				- this.difference.x,
		'wh' : this.vertical ? this.boxclone.offsetHeight
				: this.boxclone.offsetWidth
	};
	...
	for ( var i = 0; i < len; i++) {
		var boxprops = {
			'xy' : this.vertical ? this.boxes[i].offsetTop
					: this.boxes[i].offsetLeft,
			'wh' : this.vertical ? this.boxes[i].offsetHeight
					: this.boxes[i].offsetWidth
		};
		if ((this.positive && cloneprops.xy + cloneprops.wh > boxprops.xy && cloneprops.xy < boxprops.xy)
				|| (!this.positive && cloneprops.xy < boxprops.xy && cloneprops.xy
						+ cloneprops.wh > boxprops.xy)) {
			if (this.boxes[i] == this.box) {
				return;
			}
			var sibling = this.box.nextSibling;
			while (sibling.className == null
					|| !/dbx\-box/.test(sibling.className)) {
			...
			if (this.resolution > 0) {
				if (this.box[this.vertical ? 'offsetTop' : 'offsetLeft'] < boxprops.xy) {
					...
					while (visibox.className == null
							|| !/dbx\-box/.test(visibox.className)) {
			...
			if (this.resolution > 0 && visibox != this.box) {
				var animator = new dbxAnimator(this, visibox, visipos,
...
function dbxAnimator(caller, box, pos, res, kbd, anchor) {
	...
	if (before != after) {
		...
		this.box.style.visibility = 'hidden';
		this.animateClone(clone, before, after > before ? after - before
				: 0 - (before - after), this.caller.vertical ? 'top' : 'left',
				res, kbd, anchor);
	}
};
dbxAnimator.prototype.animateClone = function(clone, current, change, dir, res,
		kbd, anchor) {
	var self = this;
	var count = 0;
	dbx.running++;
			...
			.setInterval(
					function() {
						...
						if (count == res) {
							...
							if (kbd) {
								if (anchor != null
										&& anchor.parentNode.style.visibility != 'hidden') {
									anchor.focus();
								} else if (self.caller.toggles) {
									var button = self.caller.buttons[parseInt(
											self.box.className.split('dbxid')[1],
											10)];
									if (button != null
											&& typeof button.isactive != 'undefined') {
										button.focus();
									}
								}
							}
...
if (typeof window.attachEvent != 'undefined') {
	window.attachEvent('onunload', function() {
		var ev = [ 'mousedown', 'mousemove', 'mouseup', 'mouseout', 'click',
				'keydown', 'keyup', 'focus', 'blur', 'selectstart',
				'statechange', 'boxdrag', 'boxopen', 'boxclose' ];
		var el = ev.length;
